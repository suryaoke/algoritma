...
class GenetikController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        $totalJpForDayOne = []; // Total jp untuk days_id 1 berdasarkan class_room
        foreach ($teachs as $teach) {
            // Inisialisasi total jp untuk hari pertama berdasarkan class_room jika belum ada
            if (!isset($totalJpForDayOne[$teach->class_room])) {
                $totalJpForDayOne[$teach->class_room] = 0;
            }

            for ($i = 0; $i < 2; $i++) {
                // Pilih hari berdasarkan kondisi khusus jika total jp untuk days_id 1 belum mencapai 10
                if ($totalJpForDayOne[$teach->class_room] < 10 && $teach->course->jp <= 10 - $totalJpForDayOne[$teach->class_room]) {
                    // Jika total jp belum mencapai 10 dan jp dari teach tidak melebihi sisa yang diperlukan untuk mencapai 10
                    $day = Day::where('code_days', 01)->first(); // Mengambil hari dengan id 1
                    $totalJpForDayOne[$teach->class_room] += $teach->course->jp; // Menambahkan jp teach ke total jp berdasarkan class_room
                } else {
                ...
                // Pilih ruangan dan waktu secara acak
